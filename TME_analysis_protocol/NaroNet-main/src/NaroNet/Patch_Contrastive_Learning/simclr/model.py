...
from NaroNet.Patch_Contrastive_Learning.simclr.lars_optimizer import LARSOptimizer
import NaroNet.Patch_Contrastive_Learning.simclr.model_util as model_util
import NaroNet.Patch_Contrastive_Learning.simclr.objective as obj_lib
...
def build_model_fn(model, num_classes, num_train_examples, dataset,model_dir):
  ...
  def model_fn(features, labels, mode, params=None):
    ...
    # Reshape tensor
    # features = tf.reshape(features, [features.shape[0]*features.shape[1],features.shape[2],features.shape[3],features.shape[4]]) 
    ...
    if is_training:
      ...
      features_list[0] = tf.squeeze(features_list[0]) 
      features_list[1] = tf.squeeze(features_list[1]) 
      features = tf.concat(features_list, 0)  # (num_transforms * bsz, h, w, c)
    else:
      index = features[:,0,0,-1]
      features = features[:,:,:,:-1]
    ...
    with tf.variable_scope('base_model'):
      ...
      hiddens = model_util.projection_head_Z_dimensions(hiddens, is_training)      
    ...
    if is_training:
      if FLAGS.train_summary_steps > 0:
        ...
        summary_writer = tf2.summary.create_file_writer(model_dir)
      ...
      return tf.estimator.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=loss,
          predictions={'hiddens': hiddens, 'index': index},
