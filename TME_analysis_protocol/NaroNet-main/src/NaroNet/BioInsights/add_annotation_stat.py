...
def stat_test(
    ...
    **stats_params
    ...
    if test == 'Levene':
        ...
        if len(box_data1)==len(box_data2):
            if (np.array(box_data1)==np.array(box_data2)).sum()<len(box_data1):
                u_stat, pval = stats.mannwhitneyu(box_data1, box_data2, alternative='two-sided', **stats_params)
            else:
                u_stat=0
                pval=1
        else:
...
def add_stat_annotation(ax, plot='boxplot',
    ...
    def get_box_data(box_plotter, boxName):
        ...
        """
        if type(boxName) is tuple:
            cat = box_plotter.plot_hues is None and boxName or boxName[0] #Szymon
        else:
            cat = boxName
       # real_box_name = int(boxName[1]) if type(boxName) is tuple else boxName
       # cat = real_box_name
